...
export function createStarField({ latitudeDeg = 35.68, pixelRatio = 1 } = {}) {
  ...
  function update(latDeg, lstDeg, sunDirY, time, cloudiness = 0) {
    ...
    // 薄明で消える。さらに曇天では雲に隠れて見えなくなる（中程度の雲でも星は隠れる）
    const cloudFade = 1 - Math.max(0, Math.min(1, (cloudiness - 0.2) / 0.5)); // 0.2→1, 0.7→0
    const fade = fall(sunDirY, Math.sin(-2 * DEG), Math.sin(-12 * DEG)) * cloudFade;
