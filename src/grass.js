...
const BLADES_NEAR = 1500;
const BLADES_FAR = 550;
const BUILDS_PER_FRAME = 3;
const BLADE_HEIGHT = 0.7;

function createBladeGeometry() {
  // 先細り + 先端が反り返る 1 枚ブレード（高密度でばら撒くので交差板は使わない）
  const geometry = new THREE.PlaneGeometry(0.16, BLADE_HEIGHT, 1, 4);
  geometry.translate(0, BLADE_HEIGHT / 2, 0);
  const pos = geometry.attributes.position;
  for (let i = 0; i < pos.count; i++) {
    const t = pos.getY(i) / BLADE_HEIGHT;
    pos.setX(i, pos.getX(i) * (1 - t * 0.85));
    pos.setZ(i, pos.getZ(i) + t * t * 0.25); // 反り
  }
...
export function createGrassField(uniforms) {
  ...
  const tiles = new Map(); // "ix,iz" -> { mesh: InstancedMesh | null, lod: 0 | 1 }
  const buildQueue = [];
  ...
  function buildTile(ix, iz, lod) {
    ...
    for (let i = 0; i < blades; i++) {
      ...
      items.push({ x, z, h, r: rand(), s: rand(), t: rand() });
    }
  ...
  function disposeTile(key) {
    ...
    if (tile?.mesh) {
      ...
      tile.mesh.dispose(); // インスタンスバッファのみ。geometry/material は共有
    }
    tiles.delete(key);
  }
  ...
  function update(playerPos) {
    ...
    while (buildQueue.length > 0 && built < BUILDS_PER_FRAME) {
      ...
      const mesh = buildTile(ix, iz, lod);
      tiles.set(key, { mesh, lod });
