...
- **森** — 針葉樹 1,400 本・広葉樹 280 本・低木 1,800 株・岩 180 個・花 9,000 本を森林密度ノイズに従って InstancedMesh で配置。樹冠は**アルファカットアウトの葉群カード**（Canvas 2D で実行時生成した 512px 葉シルエットテクスチャ、黄緑〜深緑の色相ばらつき焼き込み）。針葉樹は**幹から放射状に伸びる枝に沿って葉房を置く枝スポーク構造**でモミの木の段々の層シルエット。広葉樹は**枝スケルトン**（幹から曲がった主枝が伸び、葉クラスタを枝先座標に直接生成して接続を保証）＋**丸い小葉のテクスチャ**＋水平寄りの葉カードで、**シード違いの 3 変種**を配置する。法線は**樹冠中心から外向きに張り替え**（球面法線）、塊全体をひとつのボリュームとして滑らかに陰影させる。影もアルファ抜き（customDepthMaterial）。樹冠内側は AO 焼き込みで陰り、逆光時は葉が透ける。幹は曲がり＋根元フレア＋**樹皮法線マップ**。岩は高 detail の正二十面体を**多重スケール変位**し、**岩肌の法線／ラフネスマップ**（プロシージャル PBR）と苔ブレンドの頂点カラーで質感を出す
...
- **空と光** — **ゴールデンアワー**（太陽高度 8°）の橙の斜光と長い影。three.js の Sky シェーダを PMREM 化して IBL（環境光）として使用。フォグは **Hoffman & Preetham (SIGGRAPH 2002) の単一散乱モデルによる空気遠近法**（fog チャンクのグローバルパッチ）— 太陽方向は暖色に明るく、反対側は青灰色に霞み、湖面・谷には朝もやが溜まる。トーンマッピングは **AgX**（ハイライトの色相保存）。葉・草の逆光透過は **Barré-Brisebois & Bouchard (GDC 2011) の薄板透過近似**。太陽光 + プレイヤー追従の 4096px シャドウ、**GTAO** で接地・葉の重なりを暗めて立体感を強調。太陽方向を向くと**光芒（サンシャフト）**と**オールドレンズ風のレンズフレア**（ゴースト列＋ハロー、樹冠の遮蔽に連動して減衰）。UnrealBloomPass + **控えめな被写界深度** + 彩度/シネマトーン/ビネット/**バレル歪曲/色収差/フィルムグレイン**のカラーグレーディング（MSAA 付きレンダーターゲット）
